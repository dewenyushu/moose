...
ComputeBlockOrientationByMisorientation::ComputeBlockOrientationByMisorientation(const InputParameters & parameters)
...
ComputeBlockOrientationByMisorientation::execute()
{
  ...
  {
    rot += _updated_rotation[_qp];
    misorient+=_misorient[_qp];
  }
  rot/=_qrule->n_points();
  misorient/=_qrule->n_points();
  ...
  EulerAngles ea = quaternionToEuler(q);
...
_grain_misorientation[_current_elem->subdomain_id()].push_back(std::make_tuple(misorient, ea.phi1, ea.Phi, ea.phi2));
}
...
ComputeBlockOrientationByMisorientation::threadJoin(const UserObject & y)
{
  ...
  const ComputeBlockOrientationByMisorientation & cbo = dynamic_cast<const ComputeBlockOrientationByMisorientation &>(y);

  for (auto it = cbo._grain_misorientation.begin(); it!=cbo._grain_misorientation.end(); ++it)
     _grain_misorientation[it->first].insert(_grain_misorientation[it->first].end(), it->second.begin(), it->second.end());
}
...
ComputeBlockOrientationByMisorientation::finalize()
{
  ...
  {
    // Sync data from all processors (gather the maximum misorientation and the corresponding EulerAngle from every processor)
...
EulerAngles
ComputeBlockOrientationByMisorientation::quaternionToEuler(const Eigen::Quaternion<Real> & q)
{
  auto yaw = std::atan2(2.0*(q.w()*q.x() + q.y()*q.z()), 1.0 - 2.0*(q.x()*q.x() + q.y()*q.y()))*(180.0 / libMesh::pi);

  auto pitch = (2.0*std::atan2(std::sqrt(1.0 + 2.0 * (q.w() * q.y() - q.x() * q.z())), std::sqrt(1.0 - 2.0 * (q.w() * q.y() - q.x() * q.z()))) - libMesh::pi/2.0)*(180.0 / libMesh::pi);

  auto roll = std::atan2(2.0*(q.w()*q.z() + q.x()*q.y()), 1.0 - 2.0*(q.z()*q.z() + q.y()*q.y()))*(180.0 / libMesh::pi);
