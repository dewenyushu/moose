...
ComputeBlockOrientationByRotation::finalize()
{
  ...
  for (std::set<SubdomainID>::const_iterator it = blocks.begin(); it != blocks.end(); ++it)
  {
    ...
    // std::cout<<*it<<" "<<_block_ea_values[*it].phi1<<" "<<_block_ea_values[*it].Phi<<" "<<_block_ea_values[*it].phi2<<std::endl;
  }
}
...
ComputeBlockOrientationByRotation::computeSubdomainEulerAngles(const SubdomainID & sid)
{
  ...
  /**
    ...
    // std::cout<<"subdomain ID : "<<sid<<"\t";
    // std::cout<<""<<q.w()<<" "<<q.x()<<" "<<q.y()<<" "<<q.z()<<"; \t";
    // auto x = q.w() * q.w() - q.x() * q.x() - q.y() * q.y() + q.z() * q.z() ;
    // auto y = 1 - std::pow(q.w() * q.w() - q.x() * q.x() - q.y() * q.y() + q.z() * q.z(), 2.0);
    // std::cout<< y <<"\t"<<x<<"\t"<<std::atan2(std::sqrt(std::abs(y)),x)<<std::endl;
