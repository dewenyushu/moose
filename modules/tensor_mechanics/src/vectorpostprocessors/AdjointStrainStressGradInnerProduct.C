...
registerMooseObject("TensorMechanicsApp", AdjointStrainStressGradInnerProduct);
...
AdjointStrainStressGradInnerProduct::computeQpInnerProduct()
{
  ...
  Real ans = -_adjoint_strain[_qp].doubleContraction(_stress_grad_uo_output[index + _qp]);

  if (_current_elem->id() == 90 && _qp == 0)
  {
    std::cout << "Adjoint strain: " << _adjoint_strain[_qp] << std::endl;
    std::cout << "Stress grad: " << _stress_grad_uo_output[index + _qp] << std::endl;
  }
