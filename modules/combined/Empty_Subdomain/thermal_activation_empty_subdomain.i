...
[Mesh]
  ...
  # =============
  # Fande:
  # we actually want an empty subdomain for the following block
  # It would be great if we can enable something like an 'EmptySubdomainGenerator' here
  [add_set3]
    type = SubdomainBoundingBoxGenerator
    input = add_set2
    block_id = 2
    bottom_left = '0.5 0.0 0.5'
    top_right = '0.6 0.1 0.51' # top_right = '0.6 0.1 0.6' will make this to run fine
    # element size = 0.1, height = 0.01 so this block actually becomes an empty, dummy block
  []
  # =============
  [moving_boundary]
