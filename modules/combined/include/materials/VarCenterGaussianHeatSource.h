//* This file is part of the MOOSE framework
//* https://www.mooseframework.org
//*
//* All rights reserved, see COPYRIGHT for full restrictions
//* https://github.com/idaholab/moose/blob/master/COPYRIGHT
//*
//* Licensed under LGPL 2.1, please see LICENSE for details
//* https://www.gnu.org/licenses/lgpl-2.1.html

#pragma once

#include "Material.h"
#include "MooseEnum.h"
#include <random>
#include <cfloat>

#include "GeometryHeightTracker.h"

/**
 * Double ellipsoid heat source distribution.
 */
class VarCenterGaussianHeatSource : public Material
{
public:
  static InputParameters validParams();

  VarCenterGaussianHeatSource(const InputParameters & parameters);

protected:
  virtual void computeQpProperties() override;

  Real computeHeatSource(const Real x, const Real y, const Real z);

  /// compute the effective radii based on the current processing parameters
  void computeEffectiveRadii();

  /// whether we use user input effective radii or from experimentally fitted formulations
  const bool & _use_input_r;

  /// whether we use the current material height as the z-coordinate of the heat source center
  const bool & _auto_heat_source_height;

  /// power
  const Real & _P;
  /// process efficienty
  const Real & _eta;
  /// material feed rate (used only when _use_input_r=false)
  const Real & _feed_rate;
  /// effective radii along three directions ()
  std::vector<Real> _r;
  /// scaling factor
  const Real & _f;
  /// factor for sampling the standard deviation
  const Real & _std_factor;

  /// heat source moving speed (scalar)
  Real _scan_speed;

  ADMaterialProperty<Real> & _volumetric_heat;

  Real _r_time_prev; // save the time when we last update the effective radii

  // name of the user object that contains the deposited material height
  std::string _material_height_uo_name;

  GeometryHeightTracker * _material_height_uo;

private:
  // center of the heat source center along three directions
  const VariableValue & _px;
  const VariableValue & _py;
  const VariableValue & _pz;

  // moving speed of the heat source center along three directions
  const VariableValue * const _vx;
  const VariableValue * const _vy;
  const VariableValue * const _vz;

  // list of the mean and std of the fitted parameters in the diameter formulation
  const std::vector<std::pair<Real, Real>> _diameter_param = {
      {0.95314595, -0.59208523},
      {-0.49753426, 0.07574844},
      {0.49324569, 0.13325939},
      {0.65076134, 0.59117914},
      {0.27564355, -0.03832367},
      {0.0904344, -0.15581355},
      {-0.19017983, -0.15591489},
      {-1.43025713, -0.05721184},
      {0.43584285, 0.1346863},
      {0.15990260757458563, 0.14783244560036998}};

  // list of the fitted parameters in the height formulation
  const std::vector<Real> _height_param = {-0.01697178,
                                           -0.61333238,
                                           0.31978383,
                                           0.15406738,
                                           0.891539,
                                           -0.32619461,
                                           -0.78659714,
                                           1.18223877,
                                           -0.55376121,
                                           0.19044780180084175};
};
