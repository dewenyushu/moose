...
GaussianHeatSourceBase::validParams()
{
  ...
  params.addRequiredParam<FunctionName>("power", "power");
                                                ...
                                                const Real time)
{
  ...
  return 2.0 * _P.value(time, dummy) * _eta * _f / libMesh::pi / _r[0] / _r[1] / _r[2] *
...
GaussianHeatSourceBase::computeEffectiveRadii(const Real time)
{
  ...
  const static Point dummy;
  Real lp = _P.value(time, dummy) / 1.0e-3 / 400.0; // input in unit 1e-3 W
