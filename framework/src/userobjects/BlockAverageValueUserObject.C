...
// MOOSE includes
#include "BlockAverageValueUserObject.h"

#include "libmesh/quadrature.h"

InputParameters
BlockAverageValueUserObject::validParams()
{
...
BlockAverageValueUserObject::BlockAverageValueUserObject(const InputParameters & parameters)
  : ElementUserObject(parameters), _qp(0)
{
}

void
BlockAverageValueUserObject::initialize()
{
...
void
BlockAverageValueUserObject::execute()
{
...
void
BlockAverageValueUserObject::finalize()
{
...
Real
BlockAverageValueUserObject::averageValue(SubdomainID block) const
{
...
void
BlockAverageValueUserObject::threadJoin(const UserObject & y)
{
  ElementUserObject::threadJoin(y);

  // We are joining with another class like this one so do a cast so we can get to it's data
  const BlockAverageValueUserObject & bav = dynamic_cast<const BlockAverageValueUserObject &>(y);
...
Real
BlockAverageValueUserObject::computeIntegral()
{
  Real sum = 0;
