...
void
ParallelDualMortarPreconditioner::getInverseD()
{
  // sorted indices
...
#ifdef DEBUG
  std::cout << "u2c_unst2st = ";
  for (auto i : u2c_unst2st)
  ...
  for (auto i : unst2st)
    if (diag_D->is_local(i.second[1]))
      if (!MooseUtils::absoluteFuzzyEqual((*_D)(i.second[1], i.second[0]), 0.0))
...
ParallelDualMortarPreconditioner::condenseSystem()
{
  ...
  // _matrix->create_submatrix(*_K2cc, lm, lm);
  // _K2cc->clear();
  // for (auto i : index_range(u2c))
  //   for (auto j : index_range(u2c))
  //   {
  //     auto row_id = u2c[i];
  //     auto col_id = u2c[j];
  //     auto val = (*_matrix)(row_id, col_id);
...
void
ParallelDualMortarPreconditioner::createSubmatrixAndPermute(
    ...
    SparseMatrix<Number> & submat)
{
  ...
  std::sort(srt_rows.begin(), srt_rows.end());
